...
每个日志或交接文档都必须有两个独立文件：英文文件使用默认文件名并作为默认入口，中文对照文件使用 `.zh-CN.md` 后缀。两份文件内容必须语义一致，只改变语言；英文版本优先维护，中文版本在同一个 commit 中同步更新。当前对应关系为：

- `DEVELOPMENT_LOG.md` / `DEVELOPMENT_LOG.zh-CN.md`
- `TEST_LOG.md` / `TEST_LOG.zh-CN.md`
- `HANDOFF.md` / `HANDOFF.zh-CN.md`
...
英文日志和交接文档是默认打开版本，中文文件只用于理解和交接辅助。日志可以包含技术名词，但面向用户的项目成果仍必须遵守本规范第 1 节的纯英文要求。
